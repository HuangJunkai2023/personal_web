import type { Hobby } from "./hobbies";

export const hobbiesZh: Hobby[] = [
  {
    id: "karting",
    title: "卡丁车",
    tagline: "不需要驾照的最接近赛车的运动。",
    description:
      "只要有机会就往卡丁车赛道跑。找到完美的刹车点、一圈一圈地压缩成绩——这种满足感很难用语言描述。本质上是把工程直觉用在 80km/h 的速度上。",
    images: [
      "/life/carting/IMG_20251002_144046.jpg",
      "/life/carting/IMG_20260111_150710.jpg",
      "/life/carting/IMG_20260117_143048_1.jpg",
    ],
  },
  {
    id: "cycling",
    title: "骑行",
    tagline: "两个轮子，没有电机，足够多的痛苦。",
    description:
      "公路骑行是我清空大脑的方式。周末的长途骑行、在 Strava 上追 KOM，偶尔在离家 60 公里的地方撞墙。自行车是一台我完全理解的机器——每个零件、每次调整都有意义。",
    images: [
      "/life/%E9%AA%91%E8%A1%8C/IMG_20250914_093515.jpg",
      "/life/%E9%AA%91%E8%A1%8C/IMG_20231003_095251.jpg",
      "/life/%E9%AA%91%E8%A1%8C/IMG_20231003_122114.jpg",
      "/life/%E9%AA%91%E8%A1%8C/IMG_20250914_113423.jpg",
      "/life/%E9%AA%91%E8%A1%8C/IMG_20260304_124259.jpg",

      "/life/%E9%AA%91%E8%A1%8C/Image_1757826870437.jpg",
    ],
  },
  {
    id: "desk-setup",
    title: "桌搭",
    tagline: "魔法发生的地方（以及永远理不完的走线）。",
    description:
      "我在桌子前花了大量时间，所以认真思考过怎么让这个空间变得舒适。自制键盘、精心挑选的外设、合适的灯光、干净的走线。这是一个永远在进行中的项目。",
    images: [
      "/life/%E6%A1%8C%E6%90%AD/%E5%AE%BF%E8%88%8D2.jpg",
      "/life/%E6%A1%8C%E6%90%AD/%E5%B7%A5%E4%BD%8D1.jpg",
      "/life/%E6%A1%8C%E6%90%AD/%E5%B7%A5%E4%BD%8D2.jpg",
      "/life/%E6%A1%8C%E6%90%AD/%E5%B7%A5%E4%BD%8D3.jpg",
      "/life/%E6%A1%8C%E6%90%AD/%E5%B7%A5%E4%BD%8D4.jpg",
      "/life/%E6%A1%8C%E6%90%AD/%E5%AE%BF%E8%88%8D1.jpg",
    ],
  },
  {
    id: "sim-racing",
    title: "模拟赛车",
    tagline: "真实的物理，虚拟的后果。",
    description:
      "去不了真实赛道的时候，就在模拟器里跑。一个像样的方向盘，在 Assetto Corsa 里练习几个小时。出乎意料地技术性——调校设定、遥测分析、赛车技巧。本质上是拿着方向盘做工程。",
    images: [
      "/life/simrace/IMG_20250811_155134.jpg",
      "/life/simrace/IMG_20251012_152454.jpg",
      "/life/simrace/IMG_20260120_211313.jpg",
      "/life/simrace/IMG_20260214_143202.jpg",
      "/life/simrace/IMG_20260214_154630.jpg",
      "/life/simrace/Image_1767799681716_703.png",
    ],
  },
  {
    id: "3d-printing",
    title: "3D 打印",
    tagline: "从 CAD 到实物，只需几个小时。",
    description:
      "3D 打印是我把数字设计变成物理现实的桥梁。从机器人零件、相机支架到自定义外壳，什么都打。调参本身就是一种工程挑战。",
    images: [
      "/life/3d%E6%89%93%E5%8D%B0/3d%E6%89%93%E5%8D%B0%E4%BB%B6.jpg",
      "/life/3d%E6%89%93%E5%8D%B0/3d%E6%89%93%E5%8D%B0.jpg",
      "/life/3d%E6%89%93%E5%8D%B0/3d%E6%89%93%E5%8D%B0%E6%91%84%E5%83%8F%E5%A4%B4%E5%9B%BA%E5%AE%9A.jpg",
    ],
  },
  {
    id: "pc-building",
    title: "电脑迷",
    tagline: "捡垃圾 E5、魔改 2080Ti，乐此不疲。",
    description:
      "配电脑、装系统、改装散热、升级配件是我的日常消遣。从零组装整机、换散热器、捡垃圾 E5 CPU、把 2080Ti 魔改到 22GB 显存——只要有散热片的东西，我大概都拆过。",
    images: [
      "/life/%E7%94%B5%E8%84%91%E8%BF%B7/IMG_20230705_151328.jpg",
      "/life/%E7%94%B5%E8%84%91%E8%BF%B7/IMG_20230803_092936.jpg",
      "/life/%E7%94%B5%E8%84%91%E8%BF%B7/IMG_20230803_173758.jpg",
      "/life/%E7%94%B5%E8%84%91%E8%BF%B7/IMG_20241213_141508.jpg",
      "/life/%E7%94%B5%E8%84%91%E8%BF%B7/IMG_20250225_233455.jpg",
      "/life/%E7%94%B5%E8%84%91%E8%BF%B7/IMG_20250612_113059.jpg",
      "/life/%E7%94%B5%E8%84%91%E8%BF%B7/IMG_20260104_151305.jpg",
      "/life/%E7%94%B5%E8%84%91%E8%BF%B7/IMG_20260105_113005.jpg",
      "/life/%E7%94%B5%E8%84%91%E8%BF%B7/IMG_20260108_093133.jpg",
    ],
  },
  {
    id: "video-editing",
    title: "视频剪辑",
    tagline: "用镜头讲述我们造的东西。",
    description:
      "为机器人队剪辑视频——招新宣传片、比赛集锦、幕后花絮。用 Premiere Pro 和 After Effects 把我们做的东西讲成一个故事。",
    links: [
      { label: "西南交大 RM 机甲大师 2025 招新 · 上篇", href: "https://www.bilibili.com/video/BV1KkbMzZEWV/?p=1" },
      { label: "西南交大 RM 机甲大师 2025 招新 · 下篇", href: "https://www.bilibili.com/video/BV1KkbMzZEWV/?p=2" },
    ],
    images: ["/life/%E8%A7%86%E9%A2%91%E5%89%AA%E8%BE%91/1743434103739.jpg"],
  },
];
